...
export function safeInteger(n: bigint): number {
  const v = Number(n);
  if (!Number.isSafeInteger(v)) throw new Error("金额超出可安全计算范围");
  return v;
}
...
export function parseMoney(value: string, currency: string): number {
  ...
  if (
    ...
    throw new Error(`此币种最多支持 ${digits} 位小数`);
...
export function convertMoney(
  ...
  if (!Number.isSafeInteger(amount) || amount < 0)
    throw new Error("金额必须为非负整数 minor units");
  const r = decimal(rate);
  if (r.n <= 0n) throw new Error("汇率必须大于零");
...
export function allocate(
  ...
  const order = entries
    .map((e, i) => ({
      i,
      rem: (BigInt(total) * e.weight) % sum,
      id: e.participantId,
    }))
    .sort((a, b) =>
      a.rem === b.rem ? a.id.localeCompare(b.id) : a.rem > b.rem ? -1 : 1,
    );
...
export function splitExpense(
  ...
  currency: string,
  method: SplitMethod,
  entries: { participantId: string; value: string }[],
) {
  if (
    !entries.length ||
    new Set(entries.map((e) => e.participantId)).size !== entries.length
  )
    throw new Error("请选择不重复的分摊参与人");
  let amounts: number[];
  if (method === "exact") {
    amounts = entries.map((e) => parseMoney(e.value, currency));
    if (amounts.reduce((s, v) => s + BigInt(v), 0n) !== BigInt(amount))
      throw new Error("指定金额之和必须等于总金额");
  } else {
    const values = entries.map((e) =>
      method === "equal" ? { n: 1n, d: 1n } : decimal(e.value),
    );
    const scale = values.reduce((m, v) => (v.d > m ? v.d : m), 1n);
    const weights = values.map((v) => v.n * (scale / v.d));
    if (
      method === "percentage" &&
      weights.reduce((s, v) => s + v, 0n) !== 100n * scale
    )
      throw new Error("百分比之和必须为 100%");
    amounts = allocate(
      amount,
      entries.map((e, i) => ({
  ...
  const bases = allocate(
    baseAmount,
    entries.map((e, i) => ({
      participantId: e.participantId,
      weight: BigInt(amounts[i]),
    })),
  );
  return entries.map((e, i) => ({
    participantId: e.participantId,
    amountMinor: amounts[i],
    baseAmountMinor: bases[i],
  }));
}
...
export function calculateBalances(
  ...
  expenses: {
    ...
    splits: { participantId: string; baseAmountMinor: number }[];
  }[],
  settlements: {
  ...
  const balances: Balance[] = [...rows.values()].map((r) => ({
    ...
    paidBase: safeInteger(r.paidBase),
    owedBase: safeInteger(r.owedBase),
    sentBase: safeInteger(r.sentBase),
    receivedBase: safeInteger(r.receivedBase),
    net: safeInteger(r.paidBase - r.owedBase + r.sentBase - r.receivedBase),
  }));
