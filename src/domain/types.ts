...
export type Member = typeof s.members.$inferSelect & { name: string; email: string };
...
export type DayPlan = Day & { items: Item[]; legs: (Leg & { alternatives: Alternative[] })[] };
export interface TripSnapshot { trip: Trip; role: Member["role"]; days: DayPlan[]; participants: Participant[]; members: Member[]; expenses: (Expense & { splits: Split[] })[]; settlements: Settlement[]; comments: Comment[]; activity: Activity[]; invites: Invite[] }
export type Mode = Leg["mode"];
export const modeLabels: Record<Mode, string> = { walking: "步行", driving: "驾车", cycling: "骑行", transit: "公交", manual: "手动" };
export const typeLabels: Record<Item["type"], string> = { place: "地点", event: "活动", hotel: "酒店", transport: "交通", border: "口岸", note: "备注" };
export const categoryLabels: Record<string, string> = { transport: "交通", food: "餐饮", hotel: "住宿", ticket: "门票", shopping: "购物", activity: "活动", other: "其他" };
