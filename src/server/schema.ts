...
export const trips = sqliteTable("trips", {
  ...
  baseCurrency: text().notNull().default("CNY"),
  baseCurrencyLockedAt: integer(),
  ...revision(),
});
export const itineraryShares = sqliteTable("itinerary_shares", {
  id: text().primaryKey(),
  tripId: text()
    .notNull()
    .unique()
...
export const days = sqliteTable(
  "days",
  {
    ...
    startMinutes: integer().notNull().default(480),
    ...revision(),
  },
...
export const items = sqliteTable(
  ...
  (t) => [index("items_order").on(t.dayId, t.position)],
);
...
export const expenses = sqliteTable(
  "expenses",
  {
    ...
    tripId: text()
      .notNull()
      .references(() => trips.id, { onDelete: "cascade" }),
    dayId: text().references(() => days.id, { onDelete: "set null" }),
    dayItemId: text().references(() => items.id, { onDelete: "set null" }),
    title: text().notNull(),
    ...
    exchangeRateToBase: text().notNull(),
    baseAmountMinor: integer().notNull(),
    splitMethod: text({
      enum: ["equal", "exact", "percentage", "shares"],
    }).notNull(),
    splitMeta: text({ mode: "json" })
      .$type<{ participantId: string; value: string }[]>()
      .notNull(),
  ...
  (t) => [index("expenses_date").on(t.tripId, t.incurredAt)],
);
export const splits = sqliteTable(
  ...
  (t) => [uniqueIndex("split_person").on(t.expenseId, t.participantId)],
);
...
export const comments = sqliteTable(
  "comments",
  {
    ...
    tripId: text()
      .notNull()
      .references(() => trips.id, { onDelete: "cascade" }),
    targetType: text({ enum: ["trip", "day_item", "expense"] }).notNull(),
  ...
  (t) => [index("comments_target").on(t.tripId, t.targetType, t.targetId)],
);
export const activity = sqliteTable(
  ...
  (t) => [index("activity_trip").on(t.tripId, t.sequence)],
);
