...
1. 创建 Trip，设置日期、时区和结算币种。应用会创建第一天，其他日期可继续添加。
2. 搜索地点添加到当天，也可手动添加活动、酒店、交通、口岸、备注或地图选点。
3. 拖动事项左侧把手排序；键盘可使用把手上的 `Alt + ↑/↓`，也可通过更多菜单上下移动。
4. 点击事项间的交通段，更换步行、驾车、骑行、公交或手动方式。展开候选后可以直接切换方案。
5. 固定活动填开始及结束时间。跨午夜选择“次日”；系统显示提前、迟到、重叠、错过及未知时间。
6. 在成员页添加尚未注册的同行者，或创建邀请。专属邀请会将注册账号绑定到原同行者，保留其费用和结算身份。
7. 在事项上点击“记一笔”，或使用费用页面，填写付款人、参与人、分摊方式和本笔汇率。转账完成后登记实际结算。
