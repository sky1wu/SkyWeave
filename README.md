...
7. 在成员页添加尚未注册的同行者，或创建邀请。专属邀请会将注册账号绑定到原同行者，保留其费用和结算身份。
   每张成员卡片可“设置备注名”，优先显示自己设置的称呼并保留原姓名供辨认。备注仅当前账号在此行程中可见，所有角色均可使用；清空后恢复原姓名。
8. 在事项上点击“记一笔”，关联账单会直接显示在事项下方；跨日移动事项时，账单关联一并跟随。转账完成后在费用页登记实际结算。
9. 在首页或行程页点击“用户设置”，修改昵称、查看登录邮箱或修改密码。昵称会显示在成员、评论与动态中；修改密码后，其他设备需要重新登录，当前设备保持登录。
10. 点击行程导航中的“查看”，进入不带编辑控件的行程手册。按天查阅时间、地址、描述与备注，展开交通详情；固定时间、跨日班次和时间冲突会保留显示。地图参考显示当天地点和已选路线，点击事项“定位”查看位置，点击地图地点返回对应安排；可切换日期、查看全日地点或收起地图。手机端地图位于正文上方，没有坐标的事项仍保留文字。有编辑权限的成员可点击“编辑行程”返回规划页。
11. 在查看页点击“导出行程图”，选择全部行程或某一天，可选择是否包含描述与备注。预览后下载 1200px 宽的 PNG；长行程自动分图，可逐张保存或下载包含全部图片的 ZIP。只读成员也可以导出。
12. 在行程顶部或查看页点击“导出行程文件”，下载可恢复的 `.skyweave.json` 文件。回到行程列表，点击“从文件导入”，选择文件后创建新行程，原行程保留。文件包含日期、每日安排、地点池、交通路线、同行者、费用分摊与结算，最大 20 MB。导入者成为新行程所有者；同行者保留为未关联账号的记账参与人，协作权限、邀请、分享链接、评论与动态不随文件转移。
...
- v0.2 可增加活动分页、按实体局部刷新、邀请身份手动合并、密码恢复、可选自动汇率、独立退款、结构化数据导入导出及更多币种。内置 AI 助手、票务和导航仍需单独确定范围；外部 Agent 已可通过 MCP 接入。
